...
##This function returns true if letter_guessed is a valid English letter that not typed before.
##If so, add this letter to all letters guessed.
...
def main():
    ...
    while(NumberOfSouls>0):
        isValidChar=False
        while(not isValidChar):
            newCharacter=input("Guess a letter: ").lower()
            isValidChar=try_update_letter_guessed(newCharacter,old_letters)
